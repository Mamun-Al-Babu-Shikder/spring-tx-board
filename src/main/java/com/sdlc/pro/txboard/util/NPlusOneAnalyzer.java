 ...
 * If any pattern repeats more than the default threshold (e.g., >= 5), we treat it as a potential N+1.
 */
public final class NPlusOneAnalyzer {
    ...
    public NPlusOneAnalyzer() {
        this(5);
    }

    public NPlusOneAnalyzer(int repeatThreshold) {
        this.repeatThreshold = Math.max(2, repeatThreshold); // at least 2 repetitions
    }

    public boolean detectPotentialNPlusOne(List<String> queries) {
        if (queries == null || queries.size() < repeatThreshold) {
            return false;
        }
    ...
    public Map<String, Integer> patternCounts(List<String> queries) {
