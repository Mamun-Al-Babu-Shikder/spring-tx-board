...
class RedisEntityInfo {
    ...
    private Field resolveIdField(Class<?> entityType) {
        ...
        if (idFields.size() != 1) {
            throw new IllegalArgumentException(String.format("The RedisEntity %s must have only one @RedisId field", entityType.getName()));
        }
    ...
    private Map<String, IndexedFieldInfo> resolveIndexedFields(Class<?> entityType) {
        ...
        if (indexedFieldInfos.isEmpty()) {
            throw new IllegalArgumentException(String.format("RedisEntity %s required at least one IndexFiled", entityType.getName()));
        }
    ...
    String getIndexName() {
        if (this.indexName == null) {
            this.indexName = this.redisEntity.indexName().trim().isEmpty() ?
    ...
    String getRecordPrefix() {
        if (this.recordPrefix == null) {
            this.recordPrefix = (redisEntity.recordPrefix().trim().isEmpty() ?
    ...
    SchemaFieldType getRedisSchemaFieldTypeOf(String fieldName) {
        ...
        if (fieldInfo == null) {
            throw new IllegalArgumentException(String.format("Field %s is not found at @RedisEntity %s", fieldName, entityType.getName()));
        }
