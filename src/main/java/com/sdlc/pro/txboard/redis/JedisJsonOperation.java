...
public class JedisJsonOperation extends AbstractRedisJsonOperation {
        ...
        if (result instanceof List<?>) {
            for (Object data : (List<?>) result) {
                if (data instanceof List<?>) {
                    List<?> sumData = (List<?>) data;
                    if (!sumData.isEmpty() && sumData.get(sumData.size() - 1) instanceof byte[]) {
                        String sum = new String((byte[]) sumData.get(sumData.size() - 1));
    ...
    private Object performCommand(RedisCommand command) {
        ...
        try {
            return connection.execute(command.getInstruction().getInstruction(), command.getArgs());
        } catch (Throwable ex) {
