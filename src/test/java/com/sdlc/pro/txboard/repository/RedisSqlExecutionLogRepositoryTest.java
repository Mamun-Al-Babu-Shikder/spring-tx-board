...
class RedisSqlExecutionLogRepositoryTest {
    ...
    static void setup() {
        ...
        Gson gson = new GsonBuilder()
                .registerTypeAdapter(Instant.class, new RedisSqlExecutionLogRepositoryTest.InstantAdapter())
                .serializeNulls()
                .create();
        ...
        redisJsonOperation.registerRedisEntityClass(RedisSqlExecutionLog.class);
        redisJsonOperation.createIndex(RedisSqlExecutionLog.class);
        logRepository = new RedisSqlExecutionLogRepository(redisJsonOperation, properties);
        List<SqlExecutionLog> sqlExecutionLogs = SqlLogUtils.createSqlExecutionLogs();
        for (SqlExecutionLog sqlExecutionLog : sqlExecutionLogs) {
            logRepository.save(sqlExecutionLog);
        }
    }
