...
public final class SqlLogUtils {
    ...
    public static List<SqlExecutionLog> createSqlExecutionLogs() {
        ...
        logs.add(new SqlExecutionLog(
                ...
                false,
                "main",
                Collections.singletonList("select * from post")
        ));
        logs.add(new SqlExecutionLog(
                ...
                true,
                "pool-3-thread-21",
                Arrays.asList("select * from account", "select * from post")
        ));
        logs.add(new SqlExecutionLog(
                ...
                false,
                "pool-1-thread-27",
                Arrays.asList("select * from post where id=1", "select * from comment")
        ));
        logs.add(new SqlExecutionLog(
                ...
                true,
                "pool-2-thread-2",
                Collections.singletonList("select * from comment where id=12")
        ));
        logs.add(new SqlExecutionLog(
                ...
                false,
                "main",
                Collections.singletonList("select * from post where id=14")
        ));
        return logs;
    }
}
