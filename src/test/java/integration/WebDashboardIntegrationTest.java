...
public class WebDashboardIntegrationTest {
    ...
    class PrePerformedTransactionLogDependentRestApiTest {
        ...
        void performTransactions() throws InterruptedException {
            ...
            for (String name : Arrays.asList("Halima", "Sharif")) {
                try {
        ...
        final class PageData {
            ...
            public PageData(int page, int size, boolean first, boolean last, int contentSize) {
                this.page = page;
                this.size = size;
                this.first = first;
                this.last = last;
        ...
        Stream<Arguments> pageDataProvider() {
            return Stream.of(
        ...
        void testTransactionLogsEndpointWithPagination(PageData data) {
            ...
            assertThat(json).isNotNull();
            assertThat(json.get("content").size()).isEqualTo(data.getContentSize());
            assertThat(json.get("totalElements").asLong()).isEqualTo(20);
            assertThat(json.get("page").asInt()).isEqualTo(data.getPage());
            assertThat(json.get("size").asInt()).isEqualTo(data.getSize());
            assertThat(json.get("totalPages").asInt()).isEqualTo(3);
            assertThat(json.get("first").asBoolean()).isEqualTo(data.isFirst());
            assertThat(json.get("last").asBoolean()).isEqualTo(data.isLast());
        }

        final Map<TransactionPhaseStatus, Long> txCountForStatus = new HashMap<>();
        {
            txCountForStatus.put(TransactionPhaseStatus.COMMITTED, 17L);
            txCountForStatus.put(TransactionPhaseStatus.ROLLED_BACK, 1L);
            txCountForStatus.put(TransactionPhaseStatus.ERRORED, 2L);
        }
        ...
        final Map<PropagationBehavior, Long> txCountForPropagation = new HashMap<>();
        {
            txCountForPropagation.put(PropagationBehavior.UNKNOWN, 0L);
            txCountForPropagation.put(PropagationBehavior.REQUIRED, 14L);
            txCountForPropagation.put(PropagationBehavior.REQUIRES_NEW, 1L);
            txCountForPropagation.put(PropagationBehavior.NESTED, 0L);
            txCountForPropagation.put(PropagationBehavior.MANDATORY, 2L);
            txCountForPropagation.put(PropagationBehavior.SUPPORTS, 1L);
            txCountForPropagation.put(PropagationBehavior.NOT_SUPPORTED, 1L);
            txCountForPropagation.put(PropagationBehavior.NEVER, 1L);
        }
        ...
        final Map<IsolationLevel, Long> txCountForIsolationLevel = new HashMap<>();
        {
            txCountForIsolationLevel.put(IsolationLevel.DEFAULT, 16L);
            txCountForIsolationLevel.put(IsolationLevel.READ_COMMITTED, 1L);
            txCountForIsolationLevel.put(IsolationLevel.READ_UNCOMMITTED, 1L);
            txCountForIsolationLevel.put(IsolationLevel.REPEATABLE_READ, 2L);
            txCountForIsolationLevel.put(IsolationLevel.SERIALIZABLE, 0L);
        }
        ...
        final Map<Boolean, Long> txCountForDBConnection = new HashMap<>();
        {
            txCountForDBConnection.put(true, 18L);
            txCountForDBConnection.put(false, 2L);
        }
        ...
        final Map<String, Long> txCountForSearchValue = new HashMap<>();
        {
            txCountForSearchValue.put("OrderService.placeOrder", 2L);
            txCountForSearchValue.put("UserService.slowUserRegistration", 2L);
            txCountForSearchValue.put("UserService.registerUserWithAware", 1L);
            txCountForSearchValue.put("anonymous", 6L);
            txCountForSearchValue.put("main", 17L);
            txCountForSearchValue.put("slow-thread", 2L);
        }
